...
sec1_re = re.compile(r"(?P<header>^\s*interface[^\n]*\n)(?P<body>(?:^(?!\s*interface).*\n?)*)", re.MULTILINE)
sec2_re = re.compile(r"(?P<header>^\s*pon-onu-mng\s+(?P<intf>\S+).*\n)(?P<body>(?:^(?!\s*pon-onu-mng).*\n?)*)", re.MULTILINE)
...
def apply_insert_line(body, rule):
    ...
    for i, ln in enumerate(lines):
        new_lines.append(ln)
        if rule["position"] == "before" and target_pattern in ln and not inserted:
            new_lines.extend([l + "\n" if not l.endswith("\n") else l for l in rule["new_lines"]])
            inserted = True
        elif rule["position"] == "after" and target_pattern in ln and not inserted:
            new_lines.extend([l + "\n" if not l.endswith("\n") else l for l in rule["new_lines"]])
            inserted = True
        elif rule["position"] == "after_last" and target_pattern in ln:
            last_idx = i
    if rule["position"] == "after_last" and "last_idx" in locals():
        insert_idx = last_idx + 1
        for l in reversed(rule["new_lines"]):
            new_lines.insert(insert_idx, l + "\n" if not l.endswith("\n") else l)
...
def process_block(header, body, interface_name, rules, block_name):
    for rule in rules:
        ...
        elif rule["type"] == "replace_multiple":
            body = apply_replace_multiple(body, rule)
        elif rule["type"] == "insert_line":
